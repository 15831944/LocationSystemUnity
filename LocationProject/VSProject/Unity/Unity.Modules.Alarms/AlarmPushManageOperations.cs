    ...
    public static void FocusDevByAlarm(DeviceAlarm alarmInfo, string msg)
    {
        ...
        try
        {
            ...
            if (depId == 0)
            {
                ...
                var devInfo = AppContext.DataClient.GetDevByid(alarmInfo.DevId);
                if (devInfo != null)
                {
            ...
            AppContext.DevManager.FocusDev(alarmInfo.DevId.ToString(), depId, result =>
            {
                ...
                if (!result)//定位失败
                {
                    ...
                    AppContext.MessageBox.ShowMessage(msgTitle);
                }
            });
        }
        catch (Exception ex)
