...
public class ArcReactor_Arc : MonoBehaviour
{
    ...
    //public static ArcReactor_Arc Instance;
    ...
    private List<Transform> shapeTransformsList = new List<Transform>();

    public void AddTransform(Transform t,int maxCount)
    {
        if (shapeTransformsList.Contains(t)) return;

        shapeTransformsList.Add(t);
        if (maxCount>0&&shapeTransformsList.Count > maxCount)
        {
            shapeTransformsList.RemoveAt(0);
        }
        shapeTransforms = shapeTransformsList.ToArray();
    }

    private List<Vector3> shapePointsList = new List<Vector3>();

    public void AddPoint(Vector3 t, int maxCount)
    {
        if (shapePointsList.Contains(t)) return;

        shapePointsList.Add(t);
        if (maxCount > 0 && shapePointsList.Count > maxCount)
        {
            shapePointsList.RemoveAt(0);
        }

        shapeTransforms = new Transform[0];
        shapePoints = shapePointsList.ToArray();
    }

    public void SetLine(Vector3 p1, Vector3 p2)
    {
        shapeTransforms = new Transform[0];
        shapePoints = new Vector3[] {p1, p2};
    }

    public void SetLine(Transform p1, Transform p2)
    {
        shapeTransforms = new Transform[] { p1, p2 }; ;
        shapePoints = new Vector3[0];
    }
		...
		if (shapePoints != null && shapeTransforms != null)
		{
			...
			for (int i = 0; i < resultingShape.Length; i++)
			{
			    if ((shapeTransforms.Length > i) && (shapeTransforms[i] != null))
			        resultingShape[i] = shapeTransforms[i].position + Offset;

			    else
			    {
			        if (shapePoints.Length > i)
			            resultingShape[i] = shapePoints[i] + Offset;
                }
			}
		}
	...
	protected void SetFlares(int n)
	{
		...
		if (arcs[n].flaresOptions.startFlare.enabled)
		{
			...
			startFlare.brightness = GetFlareBrightness(camera.transform.position,resultingShape[0],arcs[n].flaresOptions.startFlare,
			                                           arcs[n].sizeOptions.startWidthCurve.Evaluate(lifetimePos) /maxStartWidth[n]) * multiplier;
			startFlare.color = arcs[n].colorOptions.startColor.Evaluate(lifetimePos);
		}
		if (arcs[n].flaresOptions.endFlare.enabled)
		{
			...
			if (arcs[n].sizeOptions.onlyStartWidth)
				...
				endFlare.brightness = GetFlareBrightness(camera.transform.position,resultingShape[resultingShape.Length - 1],arcs[n].flaresOptions.endFlare,
				                                         arcs[n].sizeOptions.endWidthCurve.Evaluate(lifetimePos) /maxEndWidth[n]) * multiplier;
			if (arcs[n].colorOptions.onlyStartColor)			
				endFlare.color = arcs[n].colorOptions.startColor.Evaluate(lifetimePos);
			else
				endFlare.color = arcs[n].colorOptions.endColor.Evaluate(lifetimePos);
		}
	}
